...
std::string dirContent(std::string root, std::string path) {
	...
	content
		<< "</h1></header><section><table id=\"dircontent\"><tr><th>Name</th><th>Type</th><th>Size "
		   "(bytes)</th><th>Last Modified</th></tr>";

	std::vector<FileDesc> files = drl.getDirContent();
	for (std::vector<FileDesc>::iterator it = files.begin(); it != files.end(); it++) {
		content << "<tr><td><a href=\"" << it->getName() << "\">" << it->getName()
				<< "</a></td><td>" << it->getTypeName();
