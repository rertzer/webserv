		...
		if (it->find("allow_methods") != std::string::npos && locOpen == false) {
			...
			if (it->length() > 11 + 3) {
				...
				if (atoi(errorNb.c_str()) < 300 || atoi(errorNb.c_str()) > 599) {
					std::cout << "Error nb page have to be lower than 599 and upper than 300"
							  << std::endl;
			...
			if (it->find("error_page") + 15 != std::string::npos)
				errorPage = it->substr(it->find("error_page") + 15,
									   it->find(";") - it->find("error_page") - 15);
			else
				throw(ServerException());
			...
			std::string maxBodySize =
				it->substr(it->find("client_max_body_size") + 21,
			...
			*it =
				it->substr(it->find("autoindex") + 10, it->find(";") - it->find("autoindex") - 10);
			if (*it == "on")
				_autoIndex = "on";
			else if (*it == "off")
				_autoIndex = "off";
